...
# Put true tree here
tr_true <- ladderize(read.tree('sim_tr7.tree'))
tr_true$edge.length <- tr_true$edge.length / max(branching.times(tr_true))


# Load estimated trees here
est_chrono <- ladderize(read.nexus('ann_tr7.tree'))
postres <- read.nexus('test_dat7.trees')
postres <- lapply(1:length(postres), function(x) ladderize(postres[[x]]))
class(postres) <- 'multiPhylo'

#post_nodes <- round(rowSums(sapply((length(postres)-50):(length(postres)), function(x) prop.clades(est_chrono, postres[[x]]))) / 51, 2)
print('annotating posterior')
post_nodes <- round(rowSums(sapply((length(postres) - 50):(length(postres)), function(x) prop.clades(est_chrono, postres[[x]]) != 0  )) / 51, 2)
