...
get_fitted_states <- function(fit_states, sim_tree){
  ...
  node_states <- cbind( rownames(fit_states$marginal.anc), sapply(1:nrow(fit_states$marginal.anc), function(x) c('A', 'B')[which.max(c(fit_states$marginal.anc[x, 1], fit_states$marginal.anc[x, 2]))]))
  ...
  node_states_matrix[!(node_states_matrix[, 2] %in% c('A', 'B')), 2] <- sim_tree$states
